...
| T31 | **A restore overwrites the wrong thing** (another application's containers, ports, volumes or paths; a live production application by mistake) | Mandatory impact preview; collisions with anything owned by another application **block** the restore; production restores need `restore.production`, a typed confirmation and a reason; data is swapped in only after verification, the previous content is kept until the application is healthy, and any failure rolls back; the restore claims the application's workflow ID (no concurrent backup or restore) | Single-operator mode: one compromised account with `restore.production` can still overwrite an application (the previous content is deleted at commit; NAS snapshots and older recovery points remain) |
| T32 | **Cross-host restore grant outlives the restore** (the target host keeps read access to the source host's data) | The grant is per source, created for one restore and revoked by saga compensation on success, failure and cancellation; the agent uses a fresh session and closes it; grants are managed only through the internal management API | Termination skips compensation (Cancel only; ADR-0005); a leaked grant stays until revoked by hand |
| T33 | **Image substitution at restore** (a tag now points to a different or malicious image) | Missing images are pulled **by the digest recorded at capture** and verified before they are tagged | Images captured without a digest (local builds) are pulled by tag, and the preview shows this |
