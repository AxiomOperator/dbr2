...
### Added
- `dbr2-deploy.sh start|stop|restart [SERVICE...]`: `stop` keeps containers, networks and volumes (never `down`); `start` never recreates or upgrades (`up -d --no-recreate`) and refuses without the database volume; `restart` restarts in place and fails fast when containers were removed. Stopping or restarting core services waits for running backups and restores (`--force` to skip). Health and readiness are verified after `start` and `restart`.
